...
**用途边界**：本矩阵用于（a）第 0 步范围闸门与验证脚本的定义诊断（字段支持、注入机制、预算机制），（b）第 1–3 层指针/hook/预算的落点选择。boost-skill-trigger **不据此修改目标技能 frontmatter**——矩阵中"某 agent 支持/不支持某字段"只构成诊断信号，是否补写由用户走定义修复流程决定。
